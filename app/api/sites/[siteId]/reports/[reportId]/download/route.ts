...
function buildReportMd(
  ...
  // Pending improvements (with deploy instructions)
  const pendingIssues = issues.filter(i => i.status === 'PENDING')
  const severityOrder: Record<string, number> = { CRITICAL: 0, HIGH: 1, MEDIUM: 2, LOW: 3 }
  const sortedPending = [...pendingIssues].sort(
  ...
  const historicalFindings = issues.filter(i => i.status !== 'PENDING')
  if (historicalFindings.length > 0) {
    lines.push(`## ${t('report.section.historicalFindings', locale)}`, ``)

    const severityGroups: Record<string, Issue[]> = { CRITICAL: [], HIGH: [], MEDIUM: [], LOW: [] }
    for (const issue of historicalFindings) {
      const g = severityGroups[issue.severity]
      if (g) g.push(issue)
    }

    const severityHeading: Record<string, string> = {
...
export async function GET(
  ...
  const historicalFindings = issues
    .filter(i => i.status !== 'PENDING')
    .map(i => ({
      severity: i.severity,
      category: i.category,
